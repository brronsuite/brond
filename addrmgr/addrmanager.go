...
func (a *AddrManager) expireNew(bucket int) {
	...
	// Brocoind here chooses four random and just throws the oldest of
...
// We just choose the eldest. Brocoind selects 4 random entries and throws away
// the older of them.
...
func (a *AddrManager) HostToNetAddress(host string, port uint16, services wire.ServiceFlag) (*wire.NetAddress, error) {
	...
	if len(host) == 22 && host[16:] == ".onion" {
		...
		// but Tor and brocoind tend to user lowercase, so we switch
		// case here.
...
// New returns a new brocoin address manager.
