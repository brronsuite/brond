...
const (
	// BitcoindPre19 represents a brocoind version before 0.19.0.
	BitcoindPre19 BackendVersion = iota

	// BitcoindPost19 represents a brocoind version equal to or greater than
	// 0.19.0.
	BitcoindPost19
...
// Client represents a Brocoin RPC client which allows easy access to the
// various RPC methods available on a Brocoin RPC server.  Each of the wrapper
...
const (
	// bitcoind19Str is the string representation of brocoind v0.19.0.
	bitcoind19Str = "0.19.0"

	// bitcoindVersionPrefix specifies the prefix included in every brocoind
	// version exposed through GetNetworkInfo.
	bitcoindVersionPrefix = "/Satoshi:"

	// bitcoindVersionSuffix specifies the suffix included in every brocoind
	// version exposed through GetNetworkInfo.
	bitcoindVersionSuffix = "/"
)

// parseBitcoindVersion parses the brocoind version from its string
// representation.
...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	// brocoind nodes as of v0.16.0, so we'll assume the client is connected
	...
	// connected to a brocoind backend, which exposes its version through
	// GetNetworkInfo.
	networkInfo, err := c.GetNetworkInfo()
	if err != nil {
		return 0, fmt.Errorf("unable to detect brocoind version: %v", err)
	}

	// Parse the brocoind version and cache it.
	log.Debugf("Detected brocoind version: %v", networkInfo.SubVersion)
