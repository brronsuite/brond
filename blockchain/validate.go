...
func CheckTransactionSanity(tx *bronutil.Tx) error {
	...
	// as a satoshi.  One brocoin is a quantity of satoshi as defined by the
	...
	for _, txOut := range msgTx.TxOut {
		...
		// is detected and reported.  This is impossible for Brocoin, but
...
func checkBlockSanity(block *bronutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	// checks.  Brocoind builds the tree here and checks the merkle root
...
// https://github.com/brocoin/bips/blob/master/bip-0030.mediawiki and
...
func CheckTransactionInputs(tx *bronutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// brocoin is a quantity of satoshi as defined by the
		// SatoshiPerBitcoin constant.
	...
	// NOTE: brocoind checks if the transaction fees are < 0 here, but that
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *bronutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	// https://en.brocoin.it/wiki/BIP_0016 for more details.
