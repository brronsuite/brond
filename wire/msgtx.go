// Copyright (c) 2013-2016 The brronsuite developers
...
// OutPoint defines a brocoin data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new brocoin transaction outpoint point with the
...
// TxIn defines a brocoin transaction input.
type TxIn struct {
...
// NewTxIn returns a new brocoin transaction input with the provided
...
// TxOut defines a brocoin transaction output.
type TxOut struct {
	Value    int64
	PkScript []byte
}
...
// NewTxOut returns a new brocoin transaction output with the provided
...
// MsgTx implements the Message interface and represents a brocoin tx message.
...
// BtcDecode decodes r using the brocoin protocol encoding into the receiver.
...
// in that BtcDecode decodes from the brocoin wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the brocoin protocol encoding.
// This is part of the Message interface implementation.
...
// encodes the transaction to the brocoin wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new brocoin tx message that conforms to the Message
...
// writeOutPoint encodes op to the brocoin protocol encoding for an OutPoint
// to w.
...
// writeTxIn encodes ti to the brocoin protocol encoding for a transaction
// input (TxIn) to w.
...
// WriteTxOut encodes to into the brocoin protocol encoding for a transaction
// output (TxOut) to w.
//
...
// writeTxWitness encodes the brocoin protocol encoding for a transaction
