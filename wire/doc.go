...
/*
Package wire implements the brocoin wire protocol.

For the complete details of the brocoin protocol, see the official wiki entry
at https://en.brocoin.it/wiki/Protocol_specification.  The following only serves
...
supported brocoin messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Brocoin Message Overview

The brocoin protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which brocoin network it is a part of, its type, how big it is, and a checksum
...
of the currently supported brocoin messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using brocoin encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the brocoin messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official brocoin protocol wiki entry at
https://en.brocoin.it/wiki/Protocol_specification.
...
and write brocoin messages.  The following sections provide a quick overview of
...
The brocoin network is a magic number which is used to identify the start of a
message and which brocoin network the message applies to.  This package provides
...
As discussed in the brocoin message overview section, this package reads
and writes brocoin messages using a generic interface named Message.  In
...
In order to unmarshall brocoin messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a brocoin peer.  Example syntax is:
...
In order to marshall brocoin messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a brocoin peer.  Example syntax to request addresses
...
This package includes spec changes outlined by the following BIPs:
	...
	BIP0014 (https://github.com/brocoin/bips/blob/master/bip-0014.mediawiki)
	BIP0031 (https://github.com/brocoin/bips/blob/master/bip-0031.mediawiki)
	BIP0035 (https://github.com/brocoin/bips/blob/master/bip-0035.mediawiki)
	BIP0037 (https://github.com/brocoin/bips/blob/master/bip-0037.mediawiki)
	BIP0111	(https://github.com/brocoin/bips/blob/master/bip-0111.mediawiki)
	BIP0130 (https://github.com/brocoin/bips/blob/master/bip-0130.mediawiki)
	BIP0133 (https://github.com/brocoin/bips/blob/master/bip-0133.mediawiki)
*/
package wire
