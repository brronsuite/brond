...
func makeHeader(btcnet BitcoinNet, command string,
	...
	// The length of a brocoin message header is 24 bytes.
	// 4 byte magic number of the brocoin network + 12 byte command + 4 byte
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		pver    uint32     // Protocol version for wire encoding
		btcnet  BitcoinNet // Brocoin network for wire encoding
