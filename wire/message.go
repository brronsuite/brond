...
// MessageHeaderSize is the number of bytes in a brocoin message header.
// Brocoin network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
const MessageHeaderSize = 24

// CommandSize is the fixed size of all commands in the common brocoin message
...
// Commands used in brocoin message headers which describe the type of message.
const (
	CmdVersion      = "version"
	CmdVerAck       = "verack"
	CmdGetAddr      = "getaddr"
...
const (
	...
	// for the Brocoin wire protocol.
	BaseEncoding MessageEncoding = 1 << iota

	// WitnessEncoding encodes all messages other than transaction messages
	// using the default Brocoin wire protocol specification. For transaction
	// messages, the new encoding format detailed in BIP0144 will be used.
	WitnessEncoding
)

// LatestEncoding is the most recently specified encoding for the Brocoin wire
// protocol.
var LatestEncoding = WitnessEncoding

// Message is an interface that describes a brocoin message.  A type that
...
// messageHeader defines the header structure for all brocoin protocol messages.
...
// readMessageHeader reads a brocoin message header from r.
...
// WriteMessageN writes a brocoin Message to w including the necessary header
...
// WriteMessage writes a brocoin Message to w including the necessary header
...
// WriteMessageWithEncodingN writes a brocoin Message to w including the
...
// ReadMessageWithEncodingN reads, validates, and parses the next brocoin Message
// from r for the provided protocol version and brocoin network.  It returns the
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, btcnet BitcoinNet,
	...
	// Check for messages from the wrong brocoin network.
	if hdr.magic != btcnet {
...
// ReadMessageN reads, validates, and parses the next brocoin Message from r for
// the provided protocol version and brocoin network.  It returns the number of
...
// ReadMessage reads, validates, and parses the next brocoin Message from r for
// the provided protocol version and brocoin network.  It returns the parsed
